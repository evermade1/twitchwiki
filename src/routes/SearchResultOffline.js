...
function SearchResultOffline({ userInfo, onSearch }) {
    const storedStreamers = localStorage.getItem('streamers');
    const streamers = JSON.parse(storedStreamers)
    const doesStreamerExist = streamers ? streamers.some(v => v.digitId === userInfo.data[0].id) : false
    const handleMoveButtonClick = () => {
        onSearch(1, userInfo.data[0].id)
    }
        ...
        { <div className='searchResult' style={{
            ...
            <div style={{ padding: "1%", display: "flex", flexDirection: "row" }}>
                ...
                <div style={{ marginLeft: "20px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div>
                        <div style={{ fontWeight: 600, fontSize: 24, margin: 5 }}>{userInfo.data[0].display_name} ({userInfo.data[0].login})</div>
                        <div style={{ fontSize: 15, marginLeft: 5, marginTop: 10, fontStyle: "italic" }}>{userInfo.data[0].description}</div>
                    </div>
