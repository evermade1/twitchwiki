...
function App() {
  ...
  const handleMoveButtonClick = (id) => { // '이동' 버튼 클릭 시 해당 스트리머 문서로 이동
    setMoveTo(id)
  }
    ...
    <Router basename={process.env.PUBLIC_URL}>
      ...
      <Routes>
        ...
        <div><SearchResultOffline userInfo={userInfo} onSearch={SearchResultFunction} /> {streamers && <Streamers onSearch={handleMoveButtonClick} />} </div>)
          ...
          <div><div>잘못된 아이디입니다.</div> {streamers && <Streamers onSearch={handleMoveButtonClick} />}</div> : 
          <div><div>데이터가 없습니다.</div> {streamers && <Streamers onSearch={handleMoveButtonClick} />} </div>) :
          <Loading />)} /> */}
